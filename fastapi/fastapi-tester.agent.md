...
- Follow test naming convention: `test_endpoint_scenario_expectedResult`.

### BackgroundTasks Test Pattern
For routes that fire background tasks, verify the side effect:
```python
@pytest.mark.asyncio
async def test_create_user_triggers_welcome_email(client: AsyncClient, mock_email_service):
    response = await client.post("/api/v1/users", json={...})
    assert response.status_code == 201
    # BackgroundTasks run after response — assert after short wait or use testable task runner
    mock_email_service.send_welcome.assert_called_once_with(response.json()["email"])
```
