...
- [ ] `@Operation` and `@ApiResponse` OpenAPI annotations on ALL controller methods?
- [ ] Consistent naming conventions (camelCase fields, PascalCase classes, kebab-case URLs)?
- [ ] No TODO comments or placeholder code?
- [ ] No truncated methods (context window overflow)?
- [ ] `@Slf4j` logging present on service methods (INFO for success, WARN/ERROR for failures)?
- [ ] All list endpoints return `Page<T>` with `Pageable` — never unbounded `List<T>`?
- [ ] `@Lock` used correctly for concurrent-modification scenarios specified in the task?
