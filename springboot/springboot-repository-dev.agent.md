...
Use `@Version` on the entity for optimistic locking (preferred for low-contention scenarios). Use `@Lock` for explicit pessimistic locks:
```java
...
### Bulk Operations
For batch updates/deletes, use `@Modifying` to avoid loading entities into memory:
```java
@Modifying
...
```
`@Modifying` queries MUST be called from a `@Transactional` service method.
