...
When the lead specifies ISR for semi-static pages:
```tsx
export const revalidate = 60; // Revalidate every 60 seconds

export default async function ProductsPage() {
  const products = await getProducts(); // Cached and revalidated
  return <ProductList products={products} />;
}
```

### Static Params for Dynamic Routes
When the lead specifies SSG for dynamic routes:
```tsx
...
export const dynamicParams = false; // 404 for IDs not in generateStaticParams
```
